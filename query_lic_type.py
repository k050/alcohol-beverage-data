...
query = "SELECT * FROM ABC_data WHERE (county = 'LOS ANGELES' OR county = 'ORANGE' OR county = 'SAN DIEGO' OR county = 'RIVERSIDE') AND (lic_type LIKE '%41%' OR lic_type LIKE '%47%') AND (transfer_from IS NOT NULL) ORDER BY iso_date DESC LIMIT 200"
#print query
cur.execute(query)
...
with open('/home/pi/ABC/lic_type.csv', 'wb') as f:
    writer = csv.writer(f)
